...
function ItemRow({ item, styles }: { item: SaleItem; styles: ReturnType<typeof makeStyles> }) {
  return (
  ...
  return (
    <View style={styles.root}>
      ...
      <FlatList
        ...
        renderItem={({ item }) => <ItemRow item={item} styles={styles} />}
...
function makeStyles(p: ReturnType<typeof useAppPalette>) {
  return StyleSheet.create({
    ...
    metaCard: { margin: 0, borderRadius: 0, backgroundColor: p.card },
    metaContent: { gap: 4, paddingTop: 12, paddingBottom: 12 },
    metaLabel: { color: p.text3 },
    metaPayment: { color: p.text },
    voidedBanner: {
    ...
    itemQty: { color: p.text3 },
    itemTotal: { color: p.text, fontVariant: ['tabular-nums'] },
    totalRow: {
      flexDirection: 'row',
      justifyContent: 'space-between',
      paddingHorizontal: 16,
    ...
    voidedText: { textDecorationLine: 'line-through', color: p.text3 },
    errorText: { color: p.text3 },
  });
}
