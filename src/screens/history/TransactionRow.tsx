...
function makeStyles(p: ReturnType<typeof useAppPalette>) {
  return StyleSheet.create({
    ...
    time: { color: p.text3, width: 60 },
    middle: { flex: 1, gap: 4 },
    badge: { alignSelf: 'flex-start', borderRadius: 4, paddingHorizontal: 6, paddingVertical: 2 },
    badgeCash: { backgroundColor: p.softBg },
    badgeUtang: { backgroundColor: p.utangBg },
    badgeText: { color: p.text },
    customer: { color: p.text3 },
    right: { alignItems: 'flex-end', gap: 2 },
    total: { color: p.text, fontVariant: ['tabular-nums'] },
    totalVoided: { textDecorationLine: 'line-through', color: p.text3 },
    voidedLabel: { color: p.text3 },
    voidMenuDisabled: { color: p.muted },
    voidMenuDanger: { color: p.danger },
  });
}
