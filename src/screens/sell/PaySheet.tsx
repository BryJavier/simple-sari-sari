      ...
      <Portal>
        <Modal visible={visible} onDismiss={handleDismiss} contentContainerStyle={styles.container}>
          <Surface style={styles.surface}>
            <Text variant="titleMedium" style={styles.title}>Payment</Text>
            <Text variant="headlineSmall" style={styles.total}>{formatMoney(total)}</Text>
            <SegmentedButtons
              value={paymentType}
              onValueChange={(v) => setPaymentType(v as 'cash' | 'utang')}
              buttons={[{ value: 'cash', label: 'Cash' }, { value: 'utang', label: 'Utang' }]}
              style={styles.tabs}
            />
            {paymentType === 'cash' && (
              <>
                    ...
                    <View key={row.join('-')} style={styles.denomRow}>
                      {row.map((amount) => {
                        ...
                        return (
                          <Pressable
                            key={amount}
                            style={[styles.denomBtn, above ? styles.denomBtnAbove : styles.denomBtnBelow]}
                ...
                <TextInput
                  ...
                  value={tenderedText}
                  onChangeText={(t) => { if (t === '' || isValidMoneyInput(t)) setTenderedText(t); }}
                  keyboardType="decimal-pad"
                  style={styles.input}
                />
                {tenderedText !== '' && (
                  <View style={[styles.changeRow, changeCentavos >= 0 ? styles.changeRowOk : styles.changeRowShort]}>
                    <Text style={changeCentavos >= 0 ? styles.changeTextOk : styles.changeTextShort}>
                      {changeCentavos >= 0 ? `Change ${formatMoney(changeCentavos)}` : `Short by ${formatMoney(-changeCentavos)}`}
                    </Text>
                  </View>
                )}
              </>
            ...
            {paymentType === 'utang' && (
              <>
                <TextInput label="Customer name" value={customerName} onChangeText={setCustomerName} style={styles.input} autoFocus />
                <TextInput label="Contact number — optional" value={customerPhone} onChangeText={setCustomerPhone} keyboardType="phone-pad" style={styles.input} />
              </>
            )}
            <View style={styles.actions}>
              <Button onPress={handleDismiss}>Cancel</Button>
              <Button mode="contained" onPress={handleConfirm} loading={loading} disabled={!canConfirm}>Confirm</Button>
            </View>
          </Surface>
        </Modal>
      </Portal>
...
function makeStyles(p: ReturnType<typeof useAppPalette>) {
  return StyleSheet.create({
    ...
    denomLabel: { fontSize: 11, color: p.text3, textTransform: 'uppercase', letterSpacing: 0.5, marginBottom: 6 },
    denomGrid: { gap: 6, marginBottom: 12 },
    denomRow: { flexDirection: 'row', gap: 6 },
    denomBtn: { flex: 1, paddingVertical: 10, borderRadius: 6, alignItems: 'center', justifyContent: 'center', borderWidth: 1 },
    denomBtnAbove: { backgroundColor: p.softBg, borderColor: p.border },
    denomBtnBelow: { backgroundColor: p.surface, borderColor: p.borderLight, opacity: 0.4 },
    denomText: { fontSize: 14, fontWeight: '600', color: p.primary },
    changeRow: { borderRadius: 6, paddingVertical: 10, paddingHorizontal: 14, marginBottom: 12 },
